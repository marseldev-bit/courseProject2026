<?php
    ...
    if(isset($_POST['decreaseQuanity'])) {
        $sql = "SELECT * FROM cart WHERE user_id = ? AND item_id = ?";
        $stmt = $connect->prepare($sql);
        $stmt->execute([$USER['id'], $_POST['itemId']]);
        $itemQuanity = $stmt->fetch(PDO::FETCH_ASSOC);

        if($itemQuanity['quanity'] > 1) {
            $sql = "UPDATE cart SET quanity = ? WHERE user_id = ? AND item_id = ?";
            $stmt = $connect->prepare($sql);
            $stmt->execute([$itemQuanity['quanity'] - 1, $USER['id'], $_POST['itemId']]);
            echo '<script>location.href="?page=cart"</script>';
        } 
    }

    if(isset($_POST['increaseQuanity'])) {
        $sql = "SELECT * FROM cart WHERE user_id = ? AND item_id = ?";
        $stmt = $connect->prepare($sql);
        $stmt->execute([$USER['id'], $_POST['itemId']]);
        $itemQuanity = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if(isset($_POST['deleteCartItem'])) {
        ...
        $stmt = $connect->prepare($sql);
        $stmt->execute([$USER['id'], $_POST['deleteItemId']]);
        echo '<script>location.href="?page=cart"</script>';
    }
    ...
    <main>
        ...
        <div class="cart cont">
                ...
                <div class="cartItemBlock">
                    ...
                    <div class="cartItemInfo">
                        ...
                        <div class="cartItemPrice">
                            <p><?= $item['price'] * $cartItem['quanity'] ?>₽</p>
                            <div class="cartItemQuanity">
                                <form class="quanity" method="post">
                                    <button name="decreaseQuanity">-</button>
                                    <input type="hidden" name="itemId" value="<?= $cartItem['item_id'] ?>">
                                </form>
                                <h2><?= $cartItem['quanity'] ?></h2>
                                <form class="quanity" method="post">
                                    <button name="increaseQuanity">+</button>
                                    <input type="hidden" name="itemId" value="<?= $cartItem['item_id'] ?>">
                                </form>
                            </div>
                        </div>
            ...
            <div class="cartMobile">
                <div class="cartMobileHeader">
                    <img src="<?= $img['imgPath'] ?>" alt="">
                    <h1><?= $item['name'] ?></h1>
                </div>
                <div class="cartMobilePrice">
                    <p><span>Стоимость:</span> <?= $item['price'] * $cartItem['quanity'] ?>₽</p>
                        <div class="cartMobileQuanity">
                            <p><span>Количество: </span><div class="cartItemQuanity">
                            <form method="post" class="quanity">
                                <button name="decreaseQuanity">-</button>
                                <input type="hidden" name="itemId" value="<?= $cartItem['item_id'] ?>">
                            </form>
                            <h2><?= $cartItem['quanity'] ?></h2>
        ...
        <?php if($itemsCount > 0) { ?>
        <div class="clearCart cont">
            <a href="?page=cart&clear">Очистить корзину</a>
        </div>
        <?php } ?>

        <?php if($itemsCount == 0) { ?>
        <div class="emptyOrders cont">
            <h2>Корзина пуста, но это легко исправить</h2>
            <a href="?page=catalog">В каталог</a>
        </div>
        <?php } else { ?>
        <div class="totalCost cont">
            <h1>Общая сумма: <span><?= $totalCost ?>₽</span></h1>
            <a href="?page=order"><button>Перейти к оформлению</button></a>
        </div>
        <?php } ?>
        <!-- Конец "Корзина" -->
    </main>
...
<div class="modalExit active deleteCategory">
        <form method="post" class="modalExitWindow">
            <a href="?page=cart"><p>⨉</p></a>
            <div class="modalExitBody">
                <h1>Удалить из корзины товар "<?= $itemName['name'] ?>"?</h1>
                <div class="modalExitOptions">
                    <a href="?page=cart">Отмена</a>
                    <input type="hidden" value="<?= $_GET['delete'] ?>" name="deleteItemId">
                    <button name="deleteCartItem">Удалить</button>
                </div>
            </div>
        </form>
    </div>
...
<div class="modalExit active auth">
        <form method="post" class="modalExitWindow">
            <a href="?page=cart"><p>⨉</p></a>
            <div class="modalExitBody">
                <h1>Уверены, что хотите очистить коризину?</h1>
                <h2>Все товары будут удалены</h2>
                <div class="modalExitOptions">
                    <a href="?page=cart">Отмена</a>
                    <button name="clearCart">Очистить</button>
                </div>
            </div>
        </form>
    </div>
