<?php
    if(isset($_POST['addToCart']) and empty($USER['id'])) echo '<script>location.href="?page=main&auth"</script>';
    elseif(isset($_POST['addToCart']) and !empty($USER['id'])) {
        $sql = "SELECT price FROM items WHERE id = ?";
        $stmt = $connect->prepare($sql);
        $stmt->execute([$_POST['cartItemId']]);
        $price = $stmt->fetch(PDO::FETCH_ASSOC);

        $sql = "INSERT INTO cart (user_id, item_id, quanity, price) VALUES (?, ?, ?, ?)";
        $stmt = $connect->prepare($sql);
        $stmt->execute([$USER['id'], $_POST['cartItemId'], 1, $price['price']]);
        echo '<script>location.href="?page=main#catalog"</script>';
    }

    if(isset($_POST['removeFromCart'])) {
        $sql = "DELETE FROM cart WHERE user_id = ? AND item_id = ?";
        $stmt = $connect->prepare($sql);
        $stmt->execute([$USER['id'], $_POST['cartItemId']]);
        echo '<script>location.href="?page=main#catalog"</script>';
    }

    $sql = "SELECT * FROM promotions";
    $stmt = $connect->prepare($sql);
    $stmt->execute();
    $promotions = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>

<body class="mainPage">
    <!-- Шапка -->
    <?php include('pages/components/header.php'); ?>
    <!-- Конец шапки -->

    <main>
        <!-- Баннер -->
        <div class="banner">
            <img src="../assets/images/banner.png" alt="Баннер">
            <img src="../assets/images/bannerMobile.png" alt="Баннер">
            <div class="bannerText">
                <h1>Ваша партия начинается здесь</h1>
                <p>Премиум-доски, фигуры и часы. Достойный выбор для сильной игры</p>
                <p>Премиум-доски, фигуры и часы</p>
                <a href="#catalog">В каталог</a>
            </div>
        </div>
        <!-- Конец баннера  -->

        <!-- Акции -->
        <h1 class="cont titleWhite" id="advantage">О компании</h1>
        <div class="advantages cont">
            <div class="advant">
                <img src="../assets/images/adv1.png" alt="">
                <h1>Только качество</h1>
                <p>Мы отбираем шахматы из ценных пород дерева, тяжёлые фигуры и точные электронные часы. Никакого
                    пластика и брака.</p>
            </div>

            <div class="advant">
                <img src="../assets/images/adv2.png" alt="">
                <h1>Для всех уровней</h1>
                <p>У нас найдут и бюджетные дорожные наборы, и турнирные комплексы с записью партий для опытных игроков.
                </p>
            </div>

            <div class="advant">
                <img src="../assets/images/adv3.png" alt="">
                <h1>Быстрая доставка</h1>
                <p>Отправляем по всей России за 24 часа. Каждый комплект проверяем перед упаковкой, чтобы вы получили
                    идеальные шахматы.</p>
            </div>
        </div>
        <!-- Конец "Акции" -->

        <!-- Бежевая заливка фона -->
        <div class="beige" id="catalog">

            <!-- Каталог -->
            <div class="catalog cont">
                <div class="catalogHeader">
                    <h1 class="title">Каталог</h1>
                    <select>
                        <option>Все</option>
                        <?php
                        $sql = "SELECT * FROM categories";
                        $stmt = $connect->prepare($sql);
                        $stmt->execute();
                        $categories = $stmt->fetchAll(PDO::FETCH_ASSOC);

                        foreach($categories as $c) { ?>
                        <option><?= $c['name'] ?></option>
                        <?php } ?>
                    </select>
                </div>

                <div class="catalogBlock">
                    <?php
                        $sql = "SELECT * FROM items";
                        $stmt = $connect->prepare($sql);
                        $stmt->execute();
                        $items = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        $count = 0;
                        foreach($items as $item) {
                            if($count == 6) break;
                            $sql = "SELECT imgPath FROM itemGallery WHERE item_id = ?";
                            $stmt = $connect->prepare($sql);
                            $stmt->execute([$item['id']]);
                            $img = $stmt->fetch(PDO::FETCH_ASSOC);

                            $sql = "SELECT * FROM categories WHERE id = ?"; 
                            $stmt = $connect->prepare($sql);
                            $stmt->execute([$item['category_id']]);
                            $category = $stmt->fetch(PDO::FETCH_ASSOC);
                    ?>
                    <div class="card">
                        <div class="item">
                            <a href="?page=item&id=<?= $item['id'] ?>"><img src="<?= $img['imgPath'] ?>" alt="Товар">
                            <h1><?= $item['name'] ?></h1></a>
                            <p><?= $item['shortDescription'] ?></p>
                            <div class="price">
                                <h2><?= $item['price'] ?>₽</h2>
                                <a href="?page=catalog&category=<?= $category['id'] ?>"><p><?= $category['name'] ?></p></a>
                            </div>
                        </div>
                        <form class="buy" method="post">
                            <button>Купить</button>
                            <input type="hidden" name="cartItemId" value="<?= $item['id'] ?>">
                            <?php 
                            $sql = "SELECT id FROM cart WHERE user_id = ? AND item_id = ?";
                            $stmt = $connect->prepare($sql);
                            $stmt->execute([$USER['id'], $item['id']]); 
                            $inCart = $stmt->fetch(PDO::FETCH_ASSOC);
                            if($inCart) { ?> 
                            <button class="inCart" name="removeFromCart"><svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M49.1196 5.24316L21.3667 32.9959L10.8804 22.5096L0 33.3899L10.4864 43.8764L21.3667 54.7564L32.2468 43.8764L60 16.1233L49.1196 5.24316Z" fill="#4E3822"/>
                            </svg></button>
                            <?php } else { ?>
                            <button name="addToCart" name="removeFromCart"><svg width="38" height="38" viewBox="0 0 38 38" fill="#FFF8E4"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                    d="M35.5107 29.0172L32.0014 6.78654C31.8445 5.80023 31.3412 4.90212 30.5819 4.25333C29.8226 3.60454 28.857 3.24753 27.8583 3.24634H9.24519C8.24647 3.24753 7.28085 3.60454 6.52157 4.25333C5.76228 4.90212 5.25902 5.80023 5.10206 6.78654L1.59278 29.0172C1.5003 29.6151 1.53818 30.2259 1.70382 30.8078C1.86947 31.3898 2.15897 31.929 2.55247 32.3886C2.94598 32.8482 3.4342 33.2172 3.98367 33.4705C4.53314 33.7238 5.13087 33.8553 5.7359 33.856H31.3676C31.9726 33.8553 32.5704 33.7238 33.1198 33.4705C33.6693 33.2172 34.1575 32.8482 34.551 32.3886C34.9445 31.929 35.234 31.3898 35.3997 30.8078C35.5653 30.2259 35.6032 29.6151 35.5107 29.0172ZM26.2505 12.3055C25.3259 13.4587 24.1541 14.3895 22.8216 15.0291C21.489 15.6686 20.0298 16.0007 18.5517 16.0007C17.0737 16.0007 15.6145 15.6686 14.2819 15.0291C12.9494 14.3895 11.7776 13.4587 10.853 12.3055C10.722 12.1401 10.6249 11.9505 10.5673 11.7475C10.5096 11.5445 10.4925 11.3322 10.5169 11.1226C10.5413 10.913 10.6068 10.7103 10.7095 10.526C10.8123 10.3417 10.9503 10.1794 11.1158 10.0485C11.2812 9.91753 11.4709 9.82046 11.6738 9.7628C11.8768 9.70514 12.0892 9.68802 12.2987 9.71242C12.5083 9.73683 12.7111 9.80228 12.8954 9.90503C13.0796 10.0078 13.2419 10.1458 13.3728 10.3113C13.997 11.0837 14.786 11.7067 15.6821 12.1347C16.5782 12.5627 17.5587 12.7848 18.5517 12.7848C19.5448 12.7848 20.5253 12.5627 21.4214 12.1347C22.3175 11.7067 23.1065 11.0837 23.7306 10.3113C23.8616 10.1458 24.0238 10.0078 24.2081 9.90503C24.3924 9.80228 24.5952 9.73683 24.8048 9.71242C25.0143 9.68802 25.2267 9.70514 25.4297 9.7628C25.6326 9.82046 25.8223 9.91753 25.9877 10.0485C26.1532 10.1794 26.2912 10.3417 26.394 10.526C26.4967 10.7103 26.5622 10.913 26.5866 11.1226C26.611 11.3322 26.5939 11.5445 26.5362 11.7475C26.4785 11.9505 26.3815 12.1401 26.2505 12.3055Z" />
                            </svg></button>
                            <?php } ?>
                        </form>
                    </div>
                    <?php $count += 1; } ?>
                </div>

                <a href="?page=catalog" class="allItems">Все товары ></a>
            </div>
            <!-- Конец каталога -->

            <!-- Партнеры -->
            <h1 class="title cont">Наши партнеры</h1>
            <div class="partners">
                <img src="../assets/images/FIDE.png" alt="FIDE">
                <img src="../assets/images/Microsoft.png" alt="Microsoft">
                <img src="../assets/images/Chesscom.png" alt="Chesscom">
                <img src="../assets/images/lichess.png" alt="Lichess">
            </div>
            <!-- Конец "Партнеры" -->

            <!-- Часто задаваемые вопросы -->
            <h1 class="title cont faqTitle">Часто задаваемые вопросы</h1>
            <div class="faq cont">
                <div class="questionBlock active">
                    <div class="question">
                        <h1>Из какого материала сделаны шахматы в магазине «Кинг»?</h1>
                        <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.47084 0.227914C1.08954 -0.114204 0.523734 -0.0651083 0.207065 0.337572C-0.109603 0.740253 -0.0572125 1.34403 0.324082 1.68615L0.897461 0.957031L1.47084 0.227914ZM0.897461 0.957031L0.324082 1.68615L9.76591 10.1579L10.3393 9.42873L10.9127 8.69962L1.47084 0.227914L0.897461 0.957031Z"
                                fill="#FFF8E4" />
                            <path
                                d="M18.4043 0.227647C18.7856 -0.114471 19.3514 -0.0653752 19.6681 0.337305C19.9848 0.739986 19.9324 1.34376 19.5511 1.68588L18.9777 0.956765L18.4043 0.227647ZM9.53589 9.42847L8.96251 8.69935L18.4043 0.227647L18.9777 0.956765L19.5511 1.68588L10.1093 10.1576L9.53589 9.42847Z"
                                fill="#FFF8E4" />
                        </svg>
                    </div>
                    <div class="answer">
                        <p>В нашем ассортименте представлены фигуры из натурального дерева (дуб, бук, клён, орех,
                            красное дерево, самшит, эбен), а также бюджетные варианты из пластика и магнитные дорожные
                            наборы. Описание материала всегда указано в карточке товара.</p>
                    </div>
                </div>

                <div class="questionBlock">
                    <div class="question">
                        <h1>Есть ли гарантия на шахматные часы?</h1>
                        <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.47084 0.227914C1.08954 -0.114204 0.523734 -0.0651083 0.207065 0.337572C-0.109603 0.740253 -0.0572125 1.34403 0.324082 1.68615L0.897461 0.957031L1.47084 0.227914ZM0.897461 0.957031L0.324082 1.68615L9.76591 10.1579L10.3393 9.42873L10.9127 8.69962L1.47084 0.227914L0.897461 0.957031Z"
                                fill="#FFF8E4" />
                            <path
                                d="M18.4043 0.227647C18.7856 -0.114471 19.3514 -0.0653752 19.6681 0.337305C19.9848 0.739986 19.9324 1.34376 19.5511 1.68588L18.9777 0.956765L18.4043 0.227647ZM9.53589 9.42847L8.96251 8.69935L18.4043 0.227647L18.9777 0.956765L19.5511 1.68588L10.1093 10.1576L9.53589 9.42847Z"
                                fill="#FFF8E4" />
                        </svg>
                    </div>
                    <div class="answer">
                        <p>Да, на все шахматные часы предоставляется гарантия 12 месяцев. В случае заводского брака мы заменим часы или вернём деньги. Гарантия не действует при механических повреждениях и попадании жидкости.</p>
                    </div>
                </div>

                <div class="questionBlock">
                    <div class="question">
                        <h1>Можно ли вернуть товар, если он не подошёл?</h1>
                        <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.47084 0.227914C1.08954 -0.114204 0.523734 -0.0651083 0.207065 0.337572C-0.109603 0.740253 -0.0572125 1.34403 0.324082 1.68615L0.897461 0.957031L1.47084 0.227914ZM0.897461 0.957031L0.324082 1.68615L9.76591 10.1579L10.3393 9.42873L10.9127 8.69962L1.47084 0.227914L0.897461 0.957031Z"
                                fill="#FFF8E4" />
                            <path
                                d="M18.4043 0.227647C18.7856 -0.114471 19.3514 -0.0653752 19.6681 0.337305C19.9848 0.739986 19.9324 1.34376 19.5511 1.68588L18.9777 0.956765L18.4043 0.227647ZM9.53589 9.42847L8.96251 8.69935L18.4043 0.227647L18.9777 0.956765L19.5511 1.68588L10.1093 10.1576L9.53589 9.42847Z"
                                fill="#FFF8E4" />
                        </svg>
                    </div>
                    <div class="answer">
                        <p>Да, вы можете вернуть товар в течение 14 дней с момента получения, если он не был в использовании и сохранены упаковка и товарный вид. Для возврата свяжитесь с нашей поддержкой, мы поможем оформить заявку.</p>
                    </div>
                </div>

                <div class="questionBlock">
                    <div class="question">
                        <h1>Делаете ли вы гравировку на фигурах или доске?</h1>
                        <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.47084 0.227914C1.08954 -0.114204 0.523734 -0.0651083 0.207065 0.337572C-0.109603 0.740253 -0.0572125 1.34403 0.324082 1.68615L0.897461 0.957031L1.47084 0.227914ZM0.897461 0.957031L0.324082 1.68615L9.76591 10.1579L10.3393 9.42873L10.9127 8.69962L1.47084 0.227914L0.897461 0.957031Z"
                                fill="#FFF8E4" />
                            <path
                                d="M18.4043 0.227647C18.7856 -0.114471 19.3514 -0.0653752 19.6681 0.337305C19.9848 0.739986 19.9324 1.34376 19.5511 1.68588L18.9777 0.956765L18.4043 0.227647ZM9.53589 9.42847L8.96251 8.69935L18.4043 0.227647L18.9777 0.956765L19.5511 1.68588L10.1093 10.1576L9.53589 9.42847Z"
                                fill="#FFF8E4" />
                        </svg>
                    </div>
                    <div class="answer">
                        <p>К сожалению, на данный момент мы не оказываем услугу гравировки. Но планируем запустить её во втором полугодии. Следите за новостями в нашем Telegram-канале.</p>
                    </div>
                </div>

                <div class="questionBlock">
                    <div class="question">
                        <h1>Как быстро обрабатывается заказ и отправляется доставка?</h1>
                        <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.47084 0.227914C1.08954 -0.114204 0.523734 -0.0651083 0.207065 0.337572C-0.109603 0.740253 -0.0572125 1.34403 0.324082 1.68615L0.897461 0.957031L1.47084 0.227914ZM0.897461 0.957031L0.324082 1.68615L9.76591 10.1579L10.3393 9.42873L10.9127 8.69962L1.47084 0.227914L0.897461 0.957031Z"
                                fill="#FFF8E4" />
                            <path
                                d="M18.4043 0.227647C18.7856 -0.114471 19.3514 -0.0653752 19.6681 0.337305C19.9848 0.739986 19.9324 1.34376 19.5511 1.68588L18.9777 0.956765L18.4043 0.227647ZM9.53589 9.42847L8.96251 8.69935L18.4043 0.227647L18.9777 0.956765L19.5511 1.68588L10.1093 10.1576L9.53589 9.42847Z"
                                fill="#FFF8E4" />
                        </svg>
                    </div>
                    <div class="answer">
                        <p>Заказы обрабатываются в течение 24 часов после оплаты. Доставка по России занимает от 2 до 7 дней в зависимости от удалённости региона. Отправляем через СДЭК, Почту России и Boxberry. Трек-номер присылаем в SMS и на e-mail.</p>
                    </div>
                </div>
            </div>
            <!-- Конец "Часто задаваемые вопросы" -->
        </div>
        <!-- Конец бежевой заливки фона -->

        <!-- Акции -->
        <h1 class="titleWhite promotionsTitle" id="promotion">Акции</h1>
        <div class="promotionsBlock cont">
            <div class="promotions">
                <?php foreach($promotions as $prom) { ?>
                <div class="prom">
                    <img src="<?= $prom['imgPath'] ?>" alt="Акция">
                    <div class="promText">
                        <div class="promInfo">
                            <h1><?= $prom['title'] ?></h1>
                            <p><?= $prom['description'] ?></p>
                        </div>
                        <h2>до <?= $prom['date'] ?></h2>
                    </div>
                </div>
                <?php } ?>
            </div>

            <div class="slider">
                <svg class="back" width="39" height="14" viewBox="0 0 39 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M38.03 7.75813C38.5423 7.75813 38.9576 7.34285 38.9576 6.83057C38.9576 6.31829 38.5423 5.903 38.03 5.903V6.83057V7.75813ZM0.27158 6.17468C-0.0906563 6.53692 -0.0906563 7.12422 0.27158 7.48645L6.17456 13.3894C6.5368 13.7517 7.1241 13.7517 7.48634 13.3894C7.84857 13.0272 7.84857 12.4399 7.48634 12.0777L2.23924 6.83057L7.48634 1.58347C7.84857 1.22123 7.84857 0.633934 7.48634 0.271698C7.1241 -0.090539 6.5368 -0.090539 6.17456 0.271698L0.27158 6.17468ZM38.03 6.83057V5.903L0.927467 5.903V6.83057V7.75813L38.03 7.75813V6.83057Z"
                        fill="#4E3822" />
                </svg>
                <svg class="forward" width="39" height="14" viewBox="0 0 39 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M38.03 7.75813C38.5423 7.75813 38.9576 7.34285 38.9576 6.83057C38.9576 6.31829 38.5423 5.903 38.03 5.903V6.83057V7.75813ZM0.27158 6.17468C-0.0906563 6.53692 -0.0906563 7.12422 0.27158 7.48645L6.17456 13.3894C6.5368 13.7517 7.1241 13.7517 7.48634 13.3894C7.84857 13.0272 7.84857 12.4399 7.48634 12.0777L2.23924 6.83057L7.48634 1.58347C7.84857 1.22123 7.84857 0.633934 7.48634 0.271698C7.1241 -0.090539 6.5368 -0.090539 6.17456 0.271698L0.27158 6.17468ZM38.03 6.83057V5.903L0.927467 5.903V6.83057V7.75813L38.03 7.75813V6.83057Z"
                        fill="#4E3822" />
                </svg>
            </div>
        </div>
        <!-- Конец "Акции" -->
    </main>

    <!-- Подвал -->
    <footer class="cont" id="contact">
        <div class="copyright">
            <a href="#" class="logoLink"><div class="logo">
                <img src="../assets/images/logoWhite.png" alt="Лого">
                <h1>Кинг</h1>
            </div></a>
            <p>© Тимургалиев Марсель 326 ВЕБ</p>
        </div>

        <nav>
            <a href="#advantage">О компании</a>
            <a href="#catalog">Каталог</a>
            <a href="#promotion">Акции</a>
            <a href="#contact">Контакты</a>
        </nav>

        <div class="contact">
            <div class="phone">
                <h1>Телефон</h1>
                <a href="tel:+79091990407">+7 (909)-199-04-07</a>
            </div>

            <div class="email">
                <h1>E-mail</h1>
                <a href="mailto:ChessKing@gmail.com">ChessKing@gmail.com</a>
            </div>

            <div class="address">
                <h1>Адрес</h1>
                <p>Казань, Баумана 36</p>
            </div>
        </div>

        <div class="mobileCopyright">
            <div class="separateLine"></div>
            <p>© Тимургалиев Марсель 326</p>
        </div>
    </footer>
    <!-- Конец подвала -->
</body>

<?php if(isset($_GET['auth'])) { ?>
<div class="modalExit active auth">
        <form method="post" class="modalExitWindow">
            <a href="?"><p>⨉</p></a>
            <div class="modalExitBody">
                <h1>Удобнее с аккаунтом</h1>
                <h2>Чтобы добавлять товары в корзину войдите или создайте профиль. Это займёт меньше минуты.</h2>
                <div class="modalExitOptions">
                    <a href="?page=login">Войти</a>
                    <a href="?page=reg">Регистрация</a>
                </div>
            </div>
        </form>
    </div>
<?php } ?>